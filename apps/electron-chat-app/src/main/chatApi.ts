...
/** Implements the shared ChatApi contract over better-sqlite3. Runs in the Electron main process. */
export function createChatApi(db: Database.Database): ChatApi {
  const getChatRow = (chatId: string): ChatRow => {
    const row = db.prepare<[string], ChatRow>("SELECT * FROM chats WHERE id = ?").get(chatId)
  ...
  const insertExchange = (
    ...
    const now = Date.now()
    db.prepare("UPDATE chats SET updated_at = ? WHERE id = ?").run(now, chatRow.id)
    ...
    ).run(userMessageRow.id, userMessageRow.chat_id, userMessageRow.role, userMessageRow.content, userMessageRow.created_at)
  ...
  return {
    async listChats() {
      const rows = db
        .prepare<
          [],
          ChatRow
        >("SELECT * FROM chats WHERE archived_at IS NULL ORDER BY pinned DESC, updated_at DESC")
        .all()
    ...
    async listArchivedChats() {
      const rows = db
        .prepare<[], ChatRow>("SELECT * FROM chats WHERE archived_at IS NOT NULL ORDER BY archived_at DESC")
        .all()
      return rows.map(toChat)
    },
    ...
    async setChatArchived(chatId, archived) {
      db.prepare("UPDATE chats SET archived_at = ? WHERE id = ?").run(archived ? Date.now() : null, chatId)
      return toChat(getChatRow(chatId))
    },
    ...
    async renameChat(chatId, title) {
      ...
      db.prepare("UPDATE chats SET title = ? WHERE id = ?").run(
        trimmed.length > 0 ? trimmed : "New chat",
        chatId
      )
      return toChat(getChatRow(chatId))
    },
    ...
    async getMessages(chatId) {
      const rows = db
        .prepare<[string], MessageRow>("SELECT * FROM messages WHERE chat_id = ? ORDER BY created_at ASC")
        .all(chatId)
    ...
    async regenerateResponse(messageId) {
      ...
      db.prepare("UPDATE chats SET updated_at = ? WHERE id = ?").run(Date.now(), row.chat_id)
    ...
    async editMessage(messageId, content): Promise<SendMessageResult> {
      ...
      const earlierCount = db
        .prepare<[string, number], { n: number }>(
          "SELECT COUNT(*) as n FROM messages WHERE chat_id = ? AND created_at < ?"
        )
        .get(original.chat_id, original.created_at)!.n

      db.prepare("DELETE FROM messages WHERE chat_id = ? AND created_at >= ?").run(
        original.chat_id,
        original.created_at
      )
      ...
      if (earlierCount === 0) {
        ...
        db.prepare("UPDATE chats SET title = ? WHERE id = ?").run(chatRow.title, chatRow.id)
      }
